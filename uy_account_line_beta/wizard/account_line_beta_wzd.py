...
class account_line_beta_wzd(models.TransientModel):
        ...
        for line in invoice.invoice_line:
            ...
            for tax in taxes:
                if invoice.type in ('out_invoice', 'in_invoice'):
                    if tax['base_code_id'] == tax_line.base_code_id.id and tax[
                        'tax_code_id'] == tax_line.tax_code_id.id:
                        return tax['id']
                else:
                    if tax['ref_base_code_id'] == tax_line.base_code_id.id and tax[
                        'ref_tax_code_id'] == tax_line.tax_code_id.id:
                        return tax['id']
        return False
    ...
    def action_next(self):
        ...
        for row in self:
            ...
            exentos_tax_ids = row.tax_ids.filtered(
                lambda x: x.type_tax_use == 'purchase' and x.tax_group_id.name == 'EXENTOS')
            ...
            if ac_move_line_ids:
                ...
                def _do_action(self, ac_move_line, line_beta):
                    ...
                    for _r in self._group_results:
                        if _r['vat'] == ac_move_line.company_id.vat and _r['rut'] == rut and _r[
                            'line_beta'] == line_beta:
                ...
                for row_tax in row.tax_ids:
                    ...
                    if row_tax.line_beta:  # domain?
                        ...
                        for ac_move_line in ac_move_line_ids:
                            ...
                            if (ac_move_line.tax_line_id.id == row_tax.id) or (
                                    ac_move_line.tax_ids and ac_move_line.tax_ids.ids[0] == row_tax.id):
                                _do_action(self, ac_move_line, row_tax.line_beta)

                if self._group_results:
                    file_to_save.write(";".join(
                        [ustr('RUT compañía'), 'Form', ustr('Año'), 'RUT cliente', 'Fecha', ustr('Código'),
                         'Monto']) + ";\n")
                    for _r in self._group_results:
            ...
            row.write({'state': 'exported' if not not_result else 'init',
                       'file_name': 'linea_beta_' + row['year'] + row['month'] + '.' + row[
                           'file_format'] if not not_result else False,
    ...
    month = fields.Selection([('01', 'January'),
                              ...
                              ('10', 'October'),
                              ('11', 'November'),
                              ('12', 'December')], string='Mes', required=True, readonly=True,
                             states={'init': [('readonly', False)]},
                             default=ustr(date.today().month) if date.today().month >= 10 else '0' + ustr(
                                 date.today().month))
    year = fields.Selection(_get_years, string='Year', required=True, readonly=True,
                            states={'init': [('readonly', False)]}, default=ustr(date.today().year))
    file_format = fields.Selection([('txt', 'File (.txt)'), ('csv', 'File (.csv)')], 'File format', required=True,
                                   readonly=True, states={'init': [('readonly', False)]}, default='txt')
    tax_ids = fields.Many2many('account.tax', 'account_line_beta_tax_code_wzd_rel', 'wzd_id', 'tax_id',
