...
class SoapClientBase(object):
    ...
    def __attrs_request_service(self):
        if hasattr(self, 'request_' + self.request):
            if self.new_api:
                return lambda model: self.__aspect_request(getattr(self, 'request_' + self.request))(model,
                                                                                                     self.bind_method(
                                                                                                         model) or False)
            return lambda model, cr, uid, ids, context=None: self.__aspect_request(
                getattr(self, 'request_' + self.request))(model, cr, uid, ids, self.bind_method(model, cr, uid, ids,
